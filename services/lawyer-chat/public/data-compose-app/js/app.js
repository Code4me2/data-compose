...
class DataComposeApp {
    ...
    init() {
        ...
        // Listen for navigation messages from parent frame
        window.addEventListener('message', (event) => {
            if (event.data && event.data.type === 'navigate' && event.data.section) {
                this.showSection(event.data.section);
            }
        });
        
        // Sync dark mode with parent window
        this.syncDarkMode();
        setInterval(() => this.syncDarkMode(), 1000);
    ...
    syncDarkMode() {
        try {
            ...
            // If we can't access parent (cross-origin), check localStorage
